...
$('pdfBtn').onclick=()=>{if(!validate())return;const pdf=new jsPDF({unit:'mm',format:'a4'});layout((c,x,y,w,h)=>pdf.addImage(c.toDataURL('image/jpeg',.96),'JPEG',x,y,w,h,undefined,'FAST'),210,297);pdf.save('sunil-multi-hub-id-card.pdf');toast('PDF तैयार है')};
$('jpgBtn').onclick=()=>{if(!validate())return;const a=document.createElement('a');a.href=sheet.toDataURL('image/jpeg',.96);a.download='sunil-multi-hub-id-card-a4.jpg';a.click();toast('JPG तैयार है')};
