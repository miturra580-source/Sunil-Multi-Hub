...
namespace SunilAutoPrintAgent {
 public sealed class AgentApi : IDisposable {
  ...
  public AgentApi(){http.DefaultRequestHeaders.Add("apikey",PublishableKey);http.DefaultRequestHeaders.UserAgent.ParseAdd("MultiHub24AutoPrintAgent/1.2.0");}
  public async Task<AgentResponse> CallAsync(string token,string action,string printer,string jobId=null,string error=null,decimal? receivedAmount=null,string paymentReference=null){
   var body=new{token,action,printer_name=printer,platform=Environment.OSVersion.VersionString,version="1.2.0",job_id=jobId,error,received_amount=receivedAmount,payment_reference=paymentReference};
