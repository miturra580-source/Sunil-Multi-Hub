(() => {
  ...
  function isSourceUrl(url=''){
    try{return new URL(url).origin===SOURCE_ORIGIN;}catch{return false;}
  }
  ...
  function ensurePanel(){
    ...
    panel.innerHTML=`
      <div class="gj-shell">
        ...
        <div class="gj-source">Live source: SarkariResult.com.cm • नई जानकारी refresh/open पर स्वतः दिखाई देगी</div>
        ...
        <div class="gj-foot">जानकारी third-party public source से live factual format में दिखाई जाती है। Final eligibility, dates और application details official notification से verify करें।</div>
      </div>`;
    ...
    panel.querySelector('.gj-list').addEventListener('click',e=>{
      const b=e.target.closest('.gj-item'); if(!b) return;
      const url=b.dataset.url||''; if(url) loadDetail(url);
    });
    panel.querySelector('.gj-detail').addEventListener('click',e=>{
      const b=e.target.closest('[data-internal-url]'); if(!b) return;
      e.preventDefault(); loadDetail(b.dataset.internalUrl||'');
    });
    return panel;
  }

  function showFeed(){
    const p=ensurePanel(); detailMode=false; lastDetailUrl='';
    p.querySelector('.gj-tabs').style.display='flex';
    p.querySelector('.gj-search').style.display='';
    p.querySelector('.gj-back').style.display='none';
    p.querySelector('.gj-list').style.display='grid';
    p.querySelector('.gj-detail').style.display='none';
    p.querySelector('.gj-status').textContent=items.length?`${items.length} updates`:'';
    render();
  }

  function render(){
    const p=ensurePanel(); if(detailMode) return;
    ...
    p.querySelector('.gj-list').innerHTML=filtered.length?filtered.map(x=>`<button type="button" class="gj-item" data-url="${esc(x.url)}"><strong>${esc(x.title)}</strong><span>›</span></button>`).join(''):`<div class="gj-empty">कोई matching update नहीं मिला।</div>`;
  }
  ...
  async function load(force=false){
    const p=ensurePanel(); detailMode=false;
    p.querySelector('.gj-status').textContent='Latest updates loading...'; p.querySelector('.gj-list').innerHTML='';
    try{
      const base=(window.SMH_CONFIG||{}).supabaseUrl;
      const data=await api(`${base}/functions/v1/sarkari-result-feed?category=${encodeURIComponent(active)}${force?'&t='+Date.now():''}`);
      items=Array.isArray(data.items)?data.items:[];
      const when=data.fetched_at?new Date(data.fetched_at).toLocaleString('en-IN'):'';
      p.querySelector('.gj-status').textContent=`${items.length} updates • ${when}`; render();
    }catch(err){
  ...
  async function loadDetail(sourceUrl,force=false){
    if(!sourceUrl) return;
    const p=ensurePanel(); detailMode=true; lastDetailUrl=sourceUrl;
    p.querySelector('.gj-tabs').style.display='none'; p.querySelector('.gj-search').style.display='none'; p.querySelector('.gj-back').style.display='';
    p.querySelector('.gj-list').style.display='none'; p.querySelector('.gj-detail').style.display='block';
    p.querySelector('.gj-detail').innerHTML='<div class="gj-empty">Details loading...</div>';
    p.querySelector('.gj-status').textContent='Live details loading...';
    try{
      const base=(window.SMH_CONFIG||{}).supabaseUrl;
      const data=await api(`${base}/functions/v1/sarkari-result-feed?mode=detail&url=${encodeURIComponent(sourceUrl)}${force?'&t='+Date.now():''}`);
      const facts=Array.isArray(data.facts)?data.facts:[];
      const links=Array.isArray(data.links)?data.links:[];
      const actionHtml=links.slice(0,30).map(l=>{
        if(isSourceUrl(l.url)) return `<a href="#" class="gj-action internal" data-internal-url="${esc(l.url)}"><span>${esc(l.label||'More details')}</span><b>›</b></a>`;
        return `<a class="gj-action" href="${esc(l.url)}" target="_blank" rel="noopener noreferrer"><span>${esc(l.label||'Official link')}</span><b>↗</b></a>`;
      }).join('');
      p.querySelector('.gj-detail').innerHTML=`<div class="gj-detail-card"><h3>${esc(data.title||'Government Update')}</h3><div class="gj-facts">${facts.slice(0,100).map(t=>`<div class="gj-fact">${esc(t)}</div>`).join('')||'<div class="gj-empty">Structured details उपलब्ध नहीं हैं।</div>'}</div>${actionHtml?`<div class="gj-actions">${actionHtml}</div>`:''}<div class="gj-note">यहाँ factual details live source से structured रूप में दिखाई जाती हैं। आवेदन/डाउनलोड जैसे official links सीधे संबंधित official website पर खुल सकते हैं।</div></div>`;
      const when=data.fetched_at?new Date(data.fetched_at).toLocaleString('en-IN'):'';
      p.querySelector('.gj-status').textContent=`Live detail • ${when}`;
    }catch(err){
      p.querySelector('.gj-status').textContent='Detail load नहीं हो सका।';
      p.querySelector('.gj-detail').innerHTML=`<div class="gj-empty">${esc(err?.message||'Please try Refresh.')}</div>`;
    }
  }
