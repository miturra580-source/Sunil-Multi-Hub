(function(){
  ...
  style.textContent=`
  .m24-doc-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}.m24-doc-actions button{border:1px solid #cfd8e6;background:#fff;color:#172033;border-radius:10px;padding:9px 12px;font-weight:800;cursor:pointer}.m24-doc-actions button.primary{background:#0b4fd8;color:#fff;border-color:#0b4fd8}.m24-camera-input{position:absolute!important;opacity:0!important;width:1px!important;height:1px!important;pointer-events:none!important}.m24-corner-overlay{position:fixed;inset:0;z-index:99999;background:rgba(8,15,28,.88);display:flex;align-items:center;justify-content:center;padding:14px}.m24-corner-card{width:min(920px,100%);max-height:94vh;overflow:auto;background:#fff;border-radius:18px;padding:14px;box-sizing:border-box}.m24-corner-head{display:flex;justify-content:space-between;gap:12px;align-items:start;margin-bottom:10px}.m24-corner-head h3{margin:0;font-size:20px}.m24-corner-head p{margin:4px 0 0;color:#667085;font-size:12px}.m24-corner-stage{position:relative;background:#111827;border-radius:14px;overflow:hidden;touch-action:none}.m24-corner-stage canvas{display:block;width:100%;height:auto}.m24-corner-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.m24-corner-actions button{flex:1 1 140px;border:1px solid #d6deea;border-radius:10px;padding:11px 12px;background:#fff;font-weight:800}.m24-corner-actions .apply{background:#0b4fd8;color:#fff;border-color:#0b4fd8}.m24-corner-help{font-size:11px;color:#667085;line-height:1.5;margin:10px 2px 0}.m24-handle-label{position:absolute;pointer-events:none;color:#fff;background:#0b4fd8;border-radius:99px;padding:3px 6px;font-size:10px;font-weight:800}
  @media(max-width:600px){.m24-corner-overlay{padding:6px}.m24-corner-card{padding:10px;border-radius:14px}.m24-corner-head h3{font-size:17px}.m24-corner-actions button{flex:1 1 46%}}
  `;
  ...
  async function openCornerEditor(file,onDone){
    ...
    overlay.innerHTML='<div class="m24-corner-card"><div class="m24-corner-head"><div><h3>✥ Corner Edit / Document Straighten</h3><p>चारों नीले points को ID के exact corners पर रखें।</p></div><button type="button" data-close style="border:0;background:#eef2f7;border-radius:9px;padding:8px 11px;font-weight:900">✕</button></div><div class="m24-corner-stage"><canvas></canvas></div><div class="m24-corner-actions"><button type="button" data-reset>Reset Corners</button><button type="button" data-auto>Auto Corners</button><button type="button" class="apply" data-apply>✓ Straighten & Crop</button></div><p class="m24-corner-help">Tip: ऊपर-बायाँ, ऊपर-दायाँ, नीचे-दायाँ और नीचे-बायाँ corner सही border पर रखें। Apply के बाद card सीधा rectangular बनेगा।</p></div>';
    document.body.appendChild(overlay);
    const canvas=overlay.querySelector('canvas'),stage=overlay.querySelector('.m24-corner-stage'),ctx=canvas.getContext('2d');
    const maxW=1200,maxH=800,scale=Math.min(maxW/img.width,maxH/img.height,1);canvas.width=Math.max(320,Math.round(img.width*scale));canvas.height=Math.max(220,Math.round(img.height*scale));
    ...
    defaults();let active=-1;
    function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(img,0,0,canvas.width,canvas.height);ctx.save();ctx.strokeStyle='#0b4fd8';ctx.lineWidth=Math.max(3,canvas.width/260);ctx.fillStyle='rgba(11,79,216,.12)';ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);for(let i=1;i<4;i++)ctx.lineTo(pts[i].x,pts[i].y);ctx.closePath();ctx.fill();ctx.stroke();pts.forEach((p,i)=>{ctx.beginPath();ctx.fillStyle='#fff';ctx.arc(p.x,p.y,12,0,Math.PI*2);ctx.fill();ctx.lineWidth=6;ctx.strokeStyle='#0b4fd8';ctx.stroke();ctx.fillStyle='#0b4fd8';ctx.font='bold 12px sans-serif';ctx.fillText(String(i+1),p.x+15,p.y-15)});ctx.restore()}
    ...
    function nearest(p){let bi=-1,bd=1e9;pts.forEach((q,i)=>{const d=(q.x-p.x)**2+(q.y-p.y)**2;if(d<bd){bd=d;bi=i}});return bd<2500?bi:-1}
    function down(e){const p=pos(e);active=nearest(p);if(active>=0)e.preventDefault()}
    function move(e){if(active<0)return;const p=pos(e);pts[active]={x:Math.max(0,Math.min(canvas.width,p.x)),y:Math.max(0,Math.min(canvas.height,p.y))};draw();e.preventDefault()}
  ...
  function attach(input,container,label){
    ...
    corner.onclick=e=>{e.preventDefault();e.stopPropagation();const f=input.files&&input.files[0];if(!f){alert('पहले '+(label||'document')+' image चुनें');return}openCornerEditor(f,corrected=>transferTo(input,corrected))};
