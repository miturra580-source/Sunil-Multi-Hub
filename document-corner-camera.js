(function(){
  ...
  async function openCornerEditor(file,onDone){
    ...
    overlay.innerHTML='<div class="m24-corner-card"><div class="m24-corner-head"><div><h3>✥ Corner Edit / Document Straighten</h3><p>चारों नीले points ID के corners पर रखें। बीच का <b>SIZE</b> point ऊपर खींचें = बड़ा, नीचे = छोटा।</p></div><button type="button" data-close style="border:0;background:#eef2f7;border-radius:9px;padding:8px 11px;font-weight:900">✕</button></div><div class="m24-corner-stage"><canvas></canvas></div><div class="m24-corner-actions"><button type="button" data-smaller>− Size</button><button type="button" data-bigger>+ Size</button><button type="button" data-reset>Reset</button><button type="button" data-auto>Auto Corners</button><button type="button" class="apply" data-apply>✓ Straighten & Crop</button></div><p class="m24-corner-help"><span class="m24-size-note">Mobile:</span> नीचे के buttons हमेशा दिखाई देंगे। Corner 1-4 border पर रखें, SIZE point से पूरी selection छोटी-बड़ी करें।</p></div>';
    ...
    defaults();let active=-1,startPts=null,startP=null,startCenter=null;
    ...
    function down(e){const p=pos(e);active=nearest(p);if(active===4){startPts=pts.map(q=>({...q}));startP=p;startCenter=center()}if(active>=0)e.preventDefault()}
    function move(e){if(active<0)return;const p=pos(e);if(active===4){const dy=startP.y-p.y;const factor=Math.max(.25,Math.min(2.8,1+dy/(canvas.height*.42)));resizeSelection(factor,startPts,startCenter)}else{pts[active]=clampPt(p)}draw();e.preventDefault()}
    function up(){active=-1;startPts=null;startP=null;startCenter=null}
