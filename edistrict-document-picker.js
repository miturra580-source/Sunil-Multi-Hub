(() => {
  ...
  function isEdistrict(){
    ...
    if(RATION_RE.test(t+' '+v)) return false;
  ...
  function enhance(){
    ...
    ensureStyles(); section.dataset.smhPicker='1'; section.classList.add('smh-doc-picker-ready');
    const docs=wraps.map((wrap,idx)=>{const input=wrap.querySelector('input[type=file]');if(!input)return null;const label=cleanText(wrap.querySelector('strong,label')?.textContent||wrap.textContent||`दस्तावेज़ ${idx+1}`);return {wrap,input,label,required:input.required||input.dataset.required==='true'};}).filter(Boolean);
    ...
    function renderList(){const active=docs.map((d,i)=>({d,i,file:d.input.files?.[0]})).filter(x=>x.file);if(!active.length){items.innerHTML='<span style="color:#98a2b3">अभी कोई दस्तावेज़ नहीं जोड़ा गया।</span>';return;}items.innerHTML='';active.forEach(({d,file})=>{const row=document.createElement('div');row.className='smh-doc-item';row.innerHTML=`<span><strong>${d.label}</strong><br><small>${file.name} • ${(file.size/1024).toFixed(1)} KB</small></span><button type="button" class="smh-doc-remove">हटाएँ</button>`;row.querySelector('button').onclick=()=>{d.input.value='';renderList();};items.appendChild(row);});}
    picker.querySelector('#smhDocAdd').onclick=()=>{const idx=Number(sel.value);if(!Number.isInteger(idx)||!docs[idx]){alert('पहले संलग्नक शीर्षक चुनें।');return;}const file=fileInput.files?.[0];if(!file){alert('पहले फाइल चुनें।');return;}const typeOk=ALLOWED.includes(file.type)||/\.(jpe?g|png)$/i.test(file.name||'');if(!typeOk){alert('केवल JPG/JPEG/PNG फाइल अपलोड करें।');fileInput.value='';return;}if(file.size>MAX_BYTES){alert('फाइल 100 KB से अधिक नहीं हो सकती।');fileInput.value='';return;}docs[idx].input.files=dataTransferWith(file);docs[idx].input.dispatchEvent(new Event('change',{bubbles:true}));fileInput.value='';sel.value='';renderList();};
    renderList();
  }
